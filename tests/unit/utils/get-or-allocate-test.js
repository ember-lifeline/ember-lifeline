...
test("allocates an array on object when property doesn't exist", function(
  assert
) {
  assert.expect(2);
...
test("allocates an object on object when property doesn't exist", function(
  assert
) {
  assert.expect(2);
...
test("allocates an array on object when property doesn't exist and returns value", function(
  assert
) {
  assert.expect(1);
...
test("doesn't allocate property when property already exists", function(
  assert
) {
  assert.expect(1);
