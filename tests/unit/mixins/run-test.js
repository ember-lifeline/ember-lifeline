...
test('registerDisposable: returns a label when a disposable is registered', function(assert) {
  ...
  let label = subject.registerDisposable(disposable);

  assert.equal(label, 0, 'label is returned');

  let otherLabel = subject.registerDisposable(disposable);
...
test('runDisposables: runs all disposables when destroying', function(assert) {
  ...
  subject.registerDisposable(disposable);
  subject.registerDisposable(disposableTheSecond);

  assert.equal(subject._registeredDisposables.length, 2, 'two disposables are registered');

  run(subject, 'destroy');

  assert.equal(subject._registeredDisposables.length, 0, 'no disposables are registered');
});
